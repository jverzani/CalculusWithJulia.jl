...
````
CalculusWithJulia.WeaveSupport.Radioq(["`(-1, 0)` and `(1, Inf)`", "`(-Inf,
 -0.577)` and `(0.577, Inf)`", "`(-Inf, -1)` and `(0,1)`"], 1, "", nothing,
 [1, 2, 3], ["`(-1, 0)` and `(1, Inf)`", "`(-Inf, -0.577)` and `(0.577, Inf
)`", "`(-Inf, -1)` and `(0,1)`"], "", false)
````
...
````
CalculusWithJulia.WeaveSupport.Radioq(["`f(x) = x <= 10   ? 35.0 : 35.0 +  
4.0 * (x-10)`", "`f(x) = x <= 35.0 ? 10.0 : 10.0 + 35.0 * (x-4)`", "`f(x) =
 x <= 4    ? 35.0 : 35.0 + 10.0 * (x-4)`"], 1, "", nothing, [1, 2, 3], ["`f
(x) = x <= 10   ? 35.0 : 35.0 +  4.0 * (x-10)`", "`f(x) = x <= 35.0 ? 10.0 
: 10.0 + 35.0 * (x-4)`", "`f(x) = x <= 4    ? 35.0 : 35.0 + 10.0 * (x-4)`"]
, "", false)
````
...
````
CalculusWithJulia.WeaveSupport.Radioq(AbstractString["It should oscillate e
venly, but instead doesn't oscillate very much near 0 and 1", L"It oscillat
es wildly, as the period is $T=2\pi/(500 \pi)$ so there are 250 oscillation
s.", L"Oddly, it looks exactly like the graph of $f(x) = \sin(2\pi x)$."], 
3, "", nothing, [1, 2, 3], AbstractString["It should oscillate evenly, but 
instead doesn't oscillate very much near 0 and 1", L"It oscillates wildly, 
as the period is $T=2\pi/(500 \pi)$ so there are 250 oscillations.", L"Oddl
y, it looks exactly like the graph of $f(x) = \sin(2\pi x)$."], "", false)
````
...
````
CalculusWithJulia.WeaveSupport.Radioq(["Yes", "No, but is still looks prett
y bad, as fitting 250 periods into a too small number of pixels is a proble
m.", "No, the graph shows clearly all 250 periods."], 2, "", nothing, [1, 2
, 3], ["Yes", "No, but is still looks pretty bad, as fitting 250 periods in
to a too small number of pixels is a problem.", "No, the graph shows clearl
...
````
CalculusWithJulia.WeaveSupport.Radioq(["The trimmed graph is not influenced
 by the vertical asymptote at 0. It is a better graph.", "There is an error
 when plotting `trimplot(f)`.", "They appear the same, using `trimplot` jus
t complicates things."], 1, "", nothing, [1, 2, 3], ["The trimmed graph is 
not influenced by the vertical asymptote at 0. It is a better graph.", "The
re is an error when plotting `trimplot(f)`.", "They appear the same, using 
`trimplot` just complicates things."], "", false)
````
...
````
CalculusWithJulia.WeaveSupport.Radioq(["Four petals, like a flower", "A str
aight line", "An ellipse", "Four sharp points, like a star"], 1, "", nothin
g, [1, 2, 3, 4], ["Four petals, like a flower", "A straight line", "An elli
pse", "Four sharp points, like a star"], "", false)
````
